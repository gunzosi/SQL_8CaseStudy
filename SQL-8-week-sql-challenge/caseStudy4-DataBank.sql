...
\c DATABASE data_bank;

DROP TABLE IF EXISTS customer_nodes CASCADE;
...
SELECT region_id, COUNT(DISTINCT customer_nodes.node_id) AS nodes_per_region
FROM customer_nodes
GROUP BY region_id;
...
SELECT region_id,
       PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY end_date - start_date)  AS median_reallocation_days,
       PERCENTILE_CONT(0.8) WITHIN GROUP (ORDER BY end_date - start_date)  AS percentile_80_reallocation_days,
...
FROM (SELECT customer_id,
             ...
             SUM(txn_amount) FILTER (WHERE txn_type = 'deposit') AS deposit_amount
...
--            ORDER BY txn_date
--            ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS closing_balance
-- FROM customer_transactions;
...
-- a. To test out a few different hypotheses - the Data Bank team wants to run an experiment where different groups of customers would be allocated data using 3 different options:
-- a.1 Option 1: data is allocated based off the amount of money at the end of the previous month
WITH end_of_month_balance AS (SELECT customer_id,
                                     LAST_VALUE(txn_amount)
                                     OVER (PARTITION BY customer_id ORDER BY txn_date RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS end_month_balance
                              FROM customer_transactions
                              WHERE DATE_TRUNC('month', txn_date) = '2020-01-31')
SELECT customer_id,
       end_month_balance
FROM end_of_month_balance;


-- a.2 Option 2: data is allocated on the average amount of money kept in the account in the previous 30 days
WITH avg_balance_30_days AS (SELECT customer_id,
                                    AVG(txn_amount)
                                    OVER (PARTITION BY customer_id ORDER BY txn_date ROWS BETWEEN 30 PRECEDING AND CURRENT ROW) AS avg_30_days_balance
                             FROM customer_transactions)
SELECT customer_id,
       avg_30_days_balance
...
-- a.3 Option 3: data is updated real-time
WITH real_time_balance AS (SELECT customer_id,
                                  txn_date,
                                  SUM(txn_amount) OVER (
...
SELECT customer_id,
       txn_date,
       running_balance
FROM real_time_balance;

-- b. For this multi-part challenge question - you have been requested to generate the following data elements to help the Data Bank team estimate how much data will need to be provisioned for each option:
-- b.1 running customer balance column that includes the impact each transaction
SELECT customer_id,
       txn_date,
       txn_type,
       txn_amount,
...
FROM customer_transactions;
-- b.2 customer balance at the end of each month
WITH running_balance AS (SELECT customer_id,
                                txn_date,
                                txn_amount,
                                SUM(txn_amount)
                                OVER (PARTITION BY customer_id ORDER BY txn_date ROWS BETWEEN
                                    UNBOUNDED PRECEDING
                                    AND
                                    CURRENT ROW) AS running_balance
                         FROM customer_transactions)
SELECT customer_id,
       DATE_TRUNC('month', txn_date)                                                                                                             AS month,
       LAST_VALUE(running_balance)
       OVER (PARTITION BY customer_id, DATE_TRUNC('month', txn_date) ORDER BY txn_date ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS end_of_month_balance
FROM running_balance;
-- b.3 minimum, average and maximum values of the running balance for each customer
WITH running_balance AS (SELECT customer_id,
                                txn_date,
                                txn_amount,
                                SUM(txn_amount)
